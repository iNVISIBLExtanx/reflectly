...
class ResponseGenerator:
    ...
    def generate_with_memory(self, text, emotion_data, memories):
        ...
        Returns:
            str: A generated response with memory references
        """
        # Get base response
        response = self.generate(text, emotion_data)
        
        # If there are memories and the emotion is negative, add a positive memory reference
        if memories and not emotion_data['is_positive']:
            memory = random.choice(memories)
            memory_reference = f"\n\nI remember that on {memory['date']}, you wrote about feeling {memory['emotion']} when {memory['summary']}. Perhaps reflecting on that positive experience might help now."
            response += memory_reference
